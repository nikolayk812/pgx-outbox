...
func TestIntegration(t *testing.T) {
	...
	pgContainer, _, err := containers.Postgres(ctx, "postgres:17.5-alpine3.22", network.Name)
	require.NoError(t, err)
	...
	localstack, _, err := containers.Localstack(ctx, "localstack/localstack:4.7.0", "sns,sqs", network.Name)
	require.NoError(t, err)
