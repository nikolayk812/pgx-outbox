...
const EventTypeOrderCreated = "order_created" // has to match the one in writer/mapper.go
...
func TestIntegration(t *testing.T) {
	...
	network, err := tcNetwork.New(t.Context())
	...
	pgContainer, _, err := containers.Postgres(t.Context(), "postgres:17.5-alpine3.22", network.Name)
	require.NoError(t, err)
	...
	localstack, _, err := containers.Localstack(t.Context(), "localstack/localstack:4.7.0", "sns,sqs", network.Name)
	require.NoError(t, err)
	...
	waitStrategy := wait.NewLogStrategy(EventTypeOrderCreated).WithOccurrence(7).WithStartupTimeout(time.Second * 200)
